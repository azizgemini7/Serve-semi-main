...
import 'package:Serve_ios/src/models/order.dart' as orde;
...
class OrderTrackingMapScreen extends StatefulWidget {
  static const routeName = '/order-tracking-map';
  @override
...
//      }
  ...
  void didChangeDependencies() {
    ...
    order = args['order'] as orde.Order;
    orderId = order.id;
    if (!calledDirections) {
      addMarker(
          'pin_3',
    ...
    _streamSubscription = _firestore
      ...
      if (event.exists) {
        ...
        if (!calledDirections) {
          ...
          await addMarker(
            'location_circle',
            Location(delegateLocation.lat, delegateLocation.lng),
            'delegate',
//          rotation: oldBearing,
//              infoWindowTitle: AppLocalizations.of(context).delegate,
          ...
          LatLng latLng1 = LatLng(_mapsProvider.currentLocation.latitude,
              _mapsProvider.currentLocation.longitude);
